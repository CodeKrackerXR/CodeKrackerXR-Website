    ...
    const fetch = async () => {
      try {
        ...
        if (snap.exists()) {
          ...
          setData({
            clue: h10["HeadphonesClue"] || "",
            creatorClue: h10["CreatorHeadphonesClue"] || "",
            notes: h10["HeadphonesFilmingNotes"] || "",
            image: gpi["GPHeadphonesImage"] || null
          });
        }
    ...
    <div className="min-h-screen w-full relative bg-black flex flex-col font-sans text-white overflow-x-hidden">
      ...
      <div className="relative z-10 flex-1 w-full max-w-4xl mx-auto p-6 flex flex-col items-center text-center">
        <h1 className="w-full relative flex items-center justify-center text-2xl md:text-7xl font-display font-black uppercase tracking-tighter border-b-4 border-vault-gold pb-4 mb-12">
          <span className="absolute left-0 text-white/40">6.</span>
        ...
        <div className="relative w-full max-w-2xl mb-12 border-4 border-vault-gold rounded-3xl overflow-hidden shadow-2xl bg-vault-panel">
          {data?.image ? <img src={data.image} alt="Headphones" className="w-full h-auto block" /> : <div className="aspect-video flex items-center justify-center text-white/20">NO IMAGE DATA</div>}
        </div>
        
        <div className="mb-16 w-full max-w-2xl flex items-center justify-center gap-6">
          <button onClick={() => onNavigateToItem("Stud Finder")} className="text-vault-gold hover:text-white transition-all transform hover:scale-125 flex-shrink-0" title="Previous Tool">
          ...
          <button onClick={() => onNavigateToItem("SpraySmoke")} className="text-vault-gold hover:text-white transition-all transform hover:scale-125 flex-shrink-0" title="Next Tool">
